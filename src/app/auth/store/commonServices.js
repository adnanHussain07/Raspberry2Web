...
import { setAllUsers, setCustomerData, setCustomerTotalCount, setDashboardDriverData, setDashboardEmiData, setProductsData, setProductsTotalCount, setFeedbackData, setFeedbackTotalCount, setNotificationTotalCount, setNotifyData, setReqColorData, setReqCusName, setReqData, setReqDriverName, setReqStateName, setReqTotalCount, setSelReqData, setUserTotalCount, setWeightData, setWeightTotalCount, setCommonInitial } from './commonData';
import { evenOrOdd, handleResponse, isEmptyObject } from './commonMethods';
import { ReqColorCodes } from './constants';
import { changeLogsData, setSharedInitial, changeLogsTotalCount, changeUsersTotalCount, changeUsersData, changeItemPagination, loadDashData, changeUsersPagination, loadUsersData } from './sharedData';
import { setAssignLoader, setCustomersLoader, setDashboardLoader, setProductsLoader, setFeedbackLoader, setNotifyLoader, setRequestLoader, setSendNotifyLoader, setUsersLoader, setWeightLoader, setLoadersInitial, setLogsLoader, setUsersListLoader, setRentUsersListLoader } from './loadersSlice';
...
export const getUsers = (body, isReg) => async (dispatch) => {
  ...
  return ds
    ...
    .then(res => {
      const data = res ? (isReg ? (res.user ? res.user : false) : (res.users ? res.users : false)) : false;
...
export const editStatusItem = (body, data, type, pagination) => async (dispatch) => {
  dispatch(setProductsLoader(true));
  return ds
...
export const exportService = (body) => async (dispatch) => {
  ...
  return ds
    ...
    .then(res => {
      ...
      if (res) {
        dispatch(handleResponse('Success', true));
        const outputFilename = `${body && body.type ? "logs" : "items"} ${Date.now()}.xlsx`;
