...
function FuseShortcuts(props) {
  ...
  const shortcuts = useSelector(({ auth }) => auth.user.data && auth.user.data.shortcuts ? auth.user.data.shortcuts : []) || [];
  ...
  React.useEffect(() => {
    ...
    if (mounted) {
      ...
      if (shortss && !isEmptyObject(shortss) &&
        shortss.shorts && shortss.shorts.length > 0) {
        ...
        const matchSettingUrl = agreedMenus
          .filter(qw => qw.id == Menus.USERS || qw.id == Menus.NOTIFICATION || qw.id == Menus.NOTIFYASSIGN)
          .length > 0 ? agreedMenus.filter(qw => qw.id == Menus.USERS ||
            qw.id == Menus.NOTIFICATION ||
            qw.id == Menus.NOTIFYASSIGN)[0].url : '';
        setSettingBgColor(window.location.pathname == matchSettingUrl ? 'rgb(209 209 209)' : '');
      } else {
        setMenus([]);
      }
    }
  ...
  function checkSetBgColor(e) {
    if (e.id && e.id == "userregister") dispatch(changeshowAddRegister(true));
    else if (e.id && e.id == "usercreate") dispatch(changeshowAddUser(true));
    else if (e.id && e.id == "itemcreate") dispatch(changeshowAddItem(true));
    else if (e.id && e.id == "changepass") dispatch(changeShowResetPass(true));
    setSettingBgColor('');
    let data = [...getMenus];
    data.map(ww => ww.bgColor = '');
      ...
      >
        ...
        >
          <IconButton
            ...
            aria-haspopup="true"
            onClick={(e) => settingClick(e, Menus.SETTING)}
            size="large"
          >
            <Icon>settings</Icon>
          </IconButton>
        ...
        >
          <>
            {getMenus.filter(rr => rr.id == Menus.USERS).length > 0 && (
              <MenuItem
              ...
              >
                ...
                <ListItemText primary={i18next.t(`navigation:MANAGEUSERS`)} />
              </MenuItem>
            )}

            {getMenus.filter(rr => rr.id == Menus.NOTIFICATION).length > 0 && (
              <MenuItem
                onClick={settingClose}
                component={Link}
                to="/apps/redc/notify"
              ...
              >
                ...
                <ListItemText primary={i18next.t(`navigation:MANAGENOTI`)} />
              </MenuItem>
            )}

            {getMenus.filter(rr => rr.id == Menus.CHANGEPASS).length > 0 && (
              <MenuItem
                onClick={() => {
                  settingClose();
                  dispatch(setShowChangePass(true));
                }}
                role="button"
              >
                <ListItemIcon className="min-w-40">
                ...
                <ListItemText primary={i18next.t(`navigation:CHANGEPASSWORD`)} />
              </MenuItem>
            )}
          </>
        </Popover> */}
